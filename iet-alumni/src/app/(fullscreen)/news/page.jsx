...
const SkeletonRow = () => (
  <div className="flex gap-6 md:gap-10 items-start animate-pulse border-b border-slate-100 pb-10">
    <div className="flex-shrink-0 hidden sm:block w-44 h-28 bg-slate-100" />
...
export default function NewsPage() {
  ...
  const visible  = isLoggedIn
    ? news
    : news.filter(n => PUBLIC_STATUSES.includes(n.status));

  const filtered = filter === "all"
    ? visible
    : visible.filter(n => n.status === filter);

  const FILTERS = isLoggedIn ? LOGGED_IN_FILTERS : ["all"];
    ...
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-14">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-slate-500 text-base mb-10">The latest updates, announcements, and highlights from IET alumni.</p>
        </motion.div>

        {/* FILTERS — only shown to logged-in users */}
        {isLoggedIn && (
        ...
        {/* ERROR */}
        {error && !loading && <p className="text-sm text-red-500 font-medium">Could not load news: {error}</p>}

        {/* EMPTY */}
        {!loading && authReady && !error && filtered.length === 0 && (
          <div className="py-20 text-center">
            <Newspaper size={40} className="text-slate-200 mx-auto mb-4" />
            <p className="text-slate-400 text-base">No news found{filter !== "all" ? ` for "${filter}"` : ""}.</p>
          </div>
        )}
        ...
        {!loading && authReady && !error && filtered.length > 0 && (
          <div className="divide-y divide-slate-100">
            {filtered.map((item, idx) => {
              const ss = STATUS_STYLES[item.status] || STATUS_STYLES.published;
              const dateStr = item.publishedAt || item.createdAt;

              return (
                <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.06 }}
                  className="flex flex-col sm:flex-row gap-6 md:gap-10 items-start group py-10">

                  {/* THUMBNAIL */}
                  <a href={`/news/${item._id}`} className="flex-shrink-0 w-full sm:w-44 md:w-52 h-32 md:h-36 overflow-hidden relative bg-slate-100">
                    {item.image?.url ? (
                      <Image src={item.image.url} alt={item.image.altText || item.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center"><Newspaper size={28} className="text-slate-300" /></div>
                    )}
                  </a>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {isLoggedIn && (
                          <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 border ${ss.bg} ${ss.text} ${ss.border}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${ss.dot}`} />{item.status}
                          </span>
                        )}
                        {item.category && (
                          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 border border-slate-200 px-2.5 py-1">{item.category}</span>
                        )}
                      </div>

                      <a href={`/news/${item._id}`} className="block">
                        <h3 className="text-xl md:text-2xl font-bold text-slate-900 leading-tight group-hover:text-[#951114] transition-colors">{item.title}</h3>
                      </a>

                      {item.excerpt && (
                        <p className="text-sm text-slate-500 line-clamp-2 leading-relaxed">{item.excerpt}</p>
                      )}
                    </div>

                    <div className="pt-4 flex items-center gap-4 flex-wrap">
                      {item.createdByName && <span className="text-xs font-bold text-slate-500">By {item.createdByName}</span>}
