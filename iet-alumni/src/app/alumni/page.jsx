...
export default function AlumniDirectory() {
  ...
  const [filters, setFilters] = useState({ search: "", bloodGroup: "", passingYear: "", company: "" });
  ...
  const fetchAlumni = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/alumni/directory?${queryParams}`, {
  ...
  return (
    // "w-full bg-white" removes the black background visibility
    <div className="w-full min-h-screen bg-white font-sans antialiased text-slate-900">
      
      {/* HEADER SECTION - Matching IET Branding */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-200 mb-12 shadow-sm">
          <div className="relative flex-1">
            <span className="absolute left-4 top-3.5 text-slate-400">🔍</span>
            <input
              type="text"
              placeholder="Search by name, skills, or company..."
              className="w-full pl-12 pr-4 py-3 rounded-2xl border border-slate-300 focus:ring-2 focus:ring-[#800000] outline-none text-slate-900 font-medium"
              onChange={(e) => setFilters(f => ({ ...f, search: e.target.value }))}
            />
          </div>
          <div className="flex gap-3">
            <select
              className="w-44 p-3 rounded-2xl border border-slate-300 bg-white text-sm font-bold text-slate-700"
                      ...
                      <div className="flex flex-col items-start md:items-end gap-2">
                        ...
                        {person.profile?.bloodGroup && (
                          <span className="text-red-600 font-black text-sm border-b-2 border-red-100">
                            🩸 Blood Group: {person.profile.bloodGroup}
                          </span>
                        )}
                      </div>
                    </div>
