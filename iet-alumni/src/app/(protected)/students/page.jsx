...
export default function StudentDirectory() {
  ...
  const [error,        setError]        = useState(null);   // 403 handling
  ...
  const fetchStudents = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/alumni/students?${queryParams}`,
        { credentials: "include" }
      );

      // Role not allowed (student trying to access)
      if (res.status === 403) {
        setError("403");
        return;
      }
  ...
  const fetchConnections = useCallback(async () => {
    try {
      ...
      const ids = new Set(
        (data.data || []).map((item) => item.user?._id?.toString()).filter(Boolean)
      );
      setConnectedIds(ids);
    } catch (e) {
  ...
  if (error === "403") {
    return (
      <div className="w-full min-h-screen bg-white flex items-center justify-center font-sans antialiased">
        <div className="text-center px-6">
          <div className="text-8xl mb-6">🔒</div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">Access Restricted</h1>
          <p className="text-slate-500 font-medium text-lg max-w-md mx-auto">
            The Student Directory is only accessible to Alumni and Admins.
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full min-h-screen bg-white font-sans antialiased text-slate-900">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <h1 className="text-5xl font-black text-slate-900 tracking-tight mb-2">Student Directory</h1>
          <p className="text-slate-600 text-lg font-medium">Browse and connect with verified students of our community.</p>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-6 py-8">

        {/* Filters — identical structure to alumni page */}
        ...
        <div className="grid grid-cols-1 gap-8">
          {loading ? (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <p className="text-slate-700 text-lg font-medium italic leading-relaxed mb-6 border-l-4 border-slate-100 pl-4">
                      "{person.profile?.bio || "A student at IET Lucknow, building skills and shaping the future."}"
                    </p>
            ...
            <div className="text-center py-32 bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-200">
              <p className="text-slate-900 text-2xl font-black">No students found.</p>
